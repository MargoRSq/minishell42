...
/*   Updated: 2022/09/25 18:47:36 by svyatoslav       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_read(char *cache, int fd)
{
	...
	while (b_read > 0)
	{
		...
		if (b_read == 0)
			break ;
		buf[b_read] = '\0';
		cache = ft_gnl_strjoin(cache, buf);
		if (ft_gnl_strchr(cache, '\n'))
			break ;
	}
	free(buf);
	return (cache);
...
char	*ft_next_line(char *cache)
{
	...
	if (cache[nidx])
	{
		while (cache[nidx] && cache[nidx] != '\n')
			nidx++;
		nidx++;
		next_line = (char *)malloc(sizeof(char) * (nidx));
		if (!next_line)
			return (NULL);
...
char	*ft_trim_line(char *cache)
{
	...
	if (cache[nidx])
	{
		while (cache[nidx] && cache[nidx] != '\n')
			nidx++;
		new_len = ft_gnl_strlen(&cache[nidx + 1]);
		...
		if (!new_cache)
			return (NULL);
		new_cache[new_len] = '\0';
		ft_gnl_strcpy(new_cache, &cache[nidx + 1], new_len);
		free(cache);
	}
	return (new_cache);
}
